...
function buildPrompt(
  ...
  return [
    ...
    ``,
    `MAP THE SEARCH INTENT onto whatever matching filters this site exposes (look at INPUTS/CONTROLS):`,
    `- role -> the keyword/search box (fill {role}).`,
    `- remote (${filters.remote}) -> a "Remote"/workplace-type filter if present (checkbox/pill/option).`,
    `- recency (posted within ${filters.postedWithinDays} days) -> a "Date posted" filter; pick the SMALLEST`,
    `  bucket that still covers ${filters.postedWithinDays} days (Past 24 hours / Past week / Past month).`,
    `- To set a checkbox/radio filter: click its pill to open it, click the option (by label or id), then`,
    `  click the "Show results"/Apply button. If instead the site filters by URL params AND you KNOW the`,
    `  real ones for THIS exact site, a single goto with them is best`,
    `  (e.g. LinkedIn: keywords=, f_WT=2 for Remote, and f_TPR for date posted where r86400=24h,`,
    `  r604800=past week, r2592000=past month — pick the SMALLEST whose window >= ${filters.postedWithinDays} days,`,
    `  so ${filters.postedWithinDays} days -> ${sinceDays(query.since) <= 1 ? "r86400" : sinceDays(query.since) <= 7 ? "r604800" : "r2592000"}).`,
